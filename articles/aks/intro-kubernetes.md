...
---
# <a name="introduction-to-azure-container-service-aks"></a>Azure Container Service for Kubernetes (AKS) 簡介

Azure Container Service for Kubernetes (AKS) 可讓您輕鬆建立、設定及管理虛擬機器的叢集，這些虛擬機器預先設定為執行容器化應用程式。 這樣可讓您使用現有技能，或運用大量且不斷成長的社群專業知識，在 Microsoft Azure 上部署及管理容器應用程式。
...
## <a name="creating-a-kubernetes-cluster-using-azure-container-service-aks"></a>使用 Azure Container Service for Kubernetes (AKS) 建立 Kubernetes 叢集
...
* 服務探索和負載平衡
* 自動化推出和復原
* 密鑰 (Secret) 和組態 (Configuration) 管理
* 儲存體協調流程
* 批次執行
